...
let welcomeMessage = () => {
  let userName = prompt("Please, enter your name."),
    greeting = "Welcome to ISDI Coders Airlines, ";
  userName === null || userName.trim() === ""
    ? alert(greeting + "stranger.")
    : alert(greeting + userName.trim() + ".");
};
...
let lastFiveDestinations = () => {
  let lastFive = flights.slice(-5),
    statement = "The destinations of the last 5 flights are ";
...
let availableFlights = () => {
  ...
  for (let i = 0; i < flights.length; i++) {
    departures.push(
      `The flight with departure: ${flights[i].from}, and destination: ${
        flights[i].to
      } has a cost of ${flights[i].cost}€ and ${
        flights[i].scale == false
          ? "does not make any stopover."
          : "makes a stopover."
      }`
    );
  }
  let win = window.open("about:blank", null, "width=400,height=300"),
    doc = win.document;
  doc.open("text/html");
...
let determineUserType = () => {
  let showFlights = () => {
    ...
    for (let flight of flights) {
      departures.push(
        `${flight.from} to ${flight.to} at a cost of ${flight.cost} ${
          flight.scale === false ? "without scale" : "with scale"
        } and and ID of ${flight.id}`
      );
    }
  ...
  let createFlight = () => {
    ...
    alert(
      `The flight with departure: ${flights[index].from}, and destination: ${
        flights[index].to
      }, which has a cost of ${flights[index].cost}€ and ${
        flights[index].scale == false
          ? "does not make any stopover."
          : "makes a stopover."
      }, have been added succesfully.`
    );
    updateIds();
    showFlights();
  };
  ...
  let deleteFlight = () => {
    ...
    if (index === -1) {
      ...
      let message = `The flight with departure: ${
        flights[index].from
      }, and destination: ${flights[index].to}, which has a cost of ${
        flights[index].cost
      }€ and ${
        flights[index].scale == false
          ? "does not make any stopover."
  ...
  if (userType === null) {
    ...
    if (userType === "USER") {
      let search = true;
      while (search) {
        let price = promptWithType(
          "Enter a price to search for flights:",
          "number"
        );
        ...
        if (matchingFlights.length > 0) {
          ...
          for (const flight of matchingFlights) {
            flightStrings.push(
              `${flight.from} to ${flight.to} for ${flight.cost}€`
            );
          }
          alert(
            `Flights with a price of ${price}€ or less:\n\n${flightStrings.join(
              "\n"
            )}`
          );
        } else {
      ...
      while (true) {
        let action = promptWithType(
          "What do you want to do? (create/delete)",
          "string"
        );
        if (action === "create") {
          if (flights.length > 15) {
            alert(
              "Cannot add more flights.You will be redirected to the flight deletion module."
            );
            deleteFlight();
          } else {
            createFlight();
